...
export async function GET(
  ...
  try {
    ...
    // We want the users that are following `userId`
    // That means: user has a record in `following` where `followingId = userId`
    ...
    return Response.json({
      success: true,
      data,
      [Misc.API_RESPONSE_MESSAGE_KEY]: "Followers list fetched successfully",
    });
  } catch (error) {
