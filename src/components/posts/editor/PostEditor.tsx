...
function PostEditor() {
  ...
  const handleFileSelected = useCallback(
    ...
    [startUpload],
  ); // Only depends on the startUpload function which has its own validation
    ...
    >
      ...
      <div className="flex justify-end gap-3 items-center">
        ...
        <AddAttachmentsButton
          ...
          disabled={onSubmitPost.isPending || attachments.length >= MAX_FILES}
        />
        <LoadingButton
          loading={onSubmitPost.isPending}
          className="min-w-20"
          onClick={handleSubmitPost}
          disabled={!postValue.trim() || isUploading}
        >
          Post
        </LoadingButton>
      </div>
      ...
      {isDragActive && (
        <div className="absolute inset-0 flex items-center justify-center rounded-2xl bg-primary/5 z-10 pointer-events-none">
          <span className="text-primary font-medium bg-background/80 px-4 py-2 rounded-lg">
